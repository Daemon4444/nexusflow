/**
 * SMS verification code service - Aliyun SMS SDK
 *
 * ====== Environment variables (.env) ======
 *
 * SMS_ACCESS_KEY_ID=your Aliyun AccessKey ID
 * SMS_ACCESS_KEY_SECRET=your Aliyun AccessKey Secret
 * SMS_SIGN_NAME=your SMS sign name (e.g. nexusflow)
 * SMS_TEMPLATE_CODE=your SMS template code (e.g. SMS_123456789)
 *
 * ====== Aliyun setup steps ======
 *
 * 1. Sign in to Aliyun console -> SMS service https://dysms.console.aliyun.com/
 * 2. Create an SMS sign (subject to review, ~1 business day)
 * 3. Create an SMS template, e.g.:
 *      Your verification code is: ${code}, valid for 5 minutes. Do not share.
 *    The template variable name must be `code`
 * 4. Get an AccessKey: https://ram.console.aliyun.com/manage/ak
 *    Recommend a RAM sub-account with only AliyunDysmsFullAccess permission
 * 5. Put the values above into the .env file
 *
 * ====== Behavior when not configured ======
 *
 * Without environment variables, the service automatically enters test mode:
 *   - Prints the verification code to the console
 *   - The fixed code 8888 is always valid
 */
...
async function sendAliyunSms(phone: string, code: string): Promise<boolean> {
  try {
    ...
    if (error.data?.Recommend) {
      console.log("[SMS] diagnostic recommendation:", error.data.Recommend);
    }
    return false;
  }
}
...
export async function sendVerificationCode(phone: string): Promise<{ success: boolean; message: string }> {
  ...
  if (isReal) {
    ...
    if (!sent) {
      return { success: false, message: "Failed to send SMS, please retry later" };
    }
  } else {
    console.log(`[SMS-TEST] phone: ${phone}, code: ${code} (test mode, 8888 also works)`);
  }
  ...
  return {
    success: true,
    message: isReal ? "Verification code sent" : `Verification code sent (test mode, code: ${code}, 8888 also works)`,
  };
}
