...
// POST /api/auth/login - login with email + verification code
router.post("/login", validateBody(LoginSchema), async (req: Request, res: Response) => {
  ...
  if (!valid) {
    res.status(401).json({ success: false, message: "Verification code is invalid or has expired" });
    return;
  }
...
// POST /api/auth/set-password - set/change password (login required)
...
router.post("/change-password", validateBody(ChangePasswordSchema), async (req: Request, res: Response) => {
  ...
  // If a password already exists, verify the old password
