...
async function requireUploadAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  res.status(401).json({ success: false, message: "Upload credential is invalid or expired" });
}
...
router.post("/", requireUploadAuth, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    // Compression failure does not block upload; continue with the original file
  }
...
router.use((err: any, _req: any, res: any, _next: any) => {
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      res.status(413).json({ success: false, message: "File too large; maximum is 100MB" });
      return;
    }
