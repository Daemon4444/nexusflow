  ...
  const alerts = providerCards.flatMap((card) => {
    ...
    if (card.health === "down") {
      rows.push({
        ...
        detail: "At least one model route is down, traffic needs to be rerouted.",
        providerId: card.providerId,
      });
    } else if (card.health === "degraded") {
      rows.push({
        level: "warning",
        title: `${card.providerName} in degraded state`,
        detail: "Cumulative consecutive failures, recommend monitoring error rate and latency.",
        providerId: card.providerId,
      });
    }
    if (card.capacityHitRate >= 80) {
      rows.push({
        level: "warning",
        title: `${card.providerName} capacity near full`,
        detail: `Current capacity hit rate ${card.capacityHitRate}% , recommend scaling up or adjusting weights.`,
        providerId: card.providerId,
      });
    }
    return rows;
