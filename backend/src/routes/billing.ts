...
/** Extract session token from request headers and validate the user */
...
// GET /api/billing/payment/config - payment configuration status (used by frontend hints)
...
// POST /api/billing/recharge - recharge (Alipay PC website payment)
router.post("/recharge", async (req: Request, res: Response) => {
  ...
  if (normalizedAmount > 10000) {
    res.status(400).json({ success: false, message: "A single recharge cannot exceed $10000" });
    return;
  }

  // method: "page" (PC website payment) | "qr" (QR-code in-person payment) | "mock" (simulated recharge)
  const payMethod = method || "page";

  if (payMethod === "mock") {
    if (!isMockPaymentAllowed()) {
      res.status(403).json({ success: false, message: "Mock recharge is not allowed in the current environment" });
      return;
    }
    // Simulated recharge (for testing)
  ...
  // Mock mode (when Alipay is not configured) credits directly
...
router.post("/alipay/notify", async (req: Request, res: Response) => {
  ...
  // 1.1 App / merchant validation (optional strict check)
  ...
  if (!Number.isFinite(totalAmount) || Math.abs(totalAmount - order.amount) > 0.01) {
    console.error(`[ALIPAY-NOTIFY] amount mismatch: order=${order.amount}, paid=${totalAmount}`);
    res.status(400).send("fail");
    return;
  }

  // 6. Parse passback params to obtain userId
  ...
  // 7. Atomically mark the order as paid (mutex gate: only one caller can succeed)
  ...
  // 8. Execute the recharge (markOrderPaid succeeded, this is the sole executor)
  ...
  if (tx) {
    ...
    console.error(`[ALIPAY-NOTIFY] order marked paid but recharge failed, manual intervention needed: user=${userId}, amount=$${totalAmount}, order=${outTradeNo}`);
    await setOrderStatus(outTradeNo, "failed");
  }

  // 9. Return success to tell Alipay to stop notifying
  res.send("success");
});
...
router.get("/order/status", async (req: Request, res: Response) => {
  ...
  // If locally already marked as paid
  if (order.status === "paid") {
    res.json({ success: true, data: { status: "paid" } });
    return;
  }

  // Actively query Alipay
  const tradeResult = await queryTradeStatus(orderNo);
  if (tradeResult.success && tradeResult.status === "TRADE_SUCCESS") {
    // Payment succeeded but the callback hasn't arrived yet, handle manually
    if (!order.processed) {
      // Atomically mark the order as paid (mutex gate: prevents concurrent recharge with the notify callback)
      const claimed = await markOrderPaid({
        orderNo,
      ...
      if (claimed) {
        ...
        if (tx) {
          ...
          console.error(`[ALIPAY-POLL] order marked paid but recharge failed, manual intervention needed: user=${order.user_id}, amount=$${order.amount}`);
          await setOrderStatus(orderNo, "failed");
        }
      } else {
        console.log(`[ALIPAY-POLL] order already processed by another path (skipping duplicate recharge): ${orderNo}`);
      }
    }
    res.json({ success: true, data: { status: "paid" } });
    return;
