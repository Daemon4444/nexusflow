...
router.post("/users/:id/balance-adjust", async (req: Request, res: Response) => {
  ...
  if (Math.abs(amountDelta) > 100000) {
    res.status(400).json({ success: false, message: "Single balance adjustment cannot exceed 100000" });
    return;
  }
  ...
  if (!tx) {
    res.status(400).json({ success: false, message: "Balance adjustment failed, please verify user exists and balance won't be negative after deduction" });
    return;
  }
  res.json({ success: true, data: tx, message: "Balance adjusted" });
});

// ========== Log query endpoint (visible to all Admins) ==========
...
router.get("/logs/:logId/detail", async (req: Request, res: Response) => {
  try {
    ...
    // Admin can view any log, no user_id check
    ...
    if (!slsClient) {
      res.json({ success: false, message: "SLS not configured" });
      return;
    }
    ...
    res.json({
      ...
      note: entry ? undefined : "Log may still be indexing (SLS delay 1-2 minutes), please retry later",
    });
  } catch (err: any) {
