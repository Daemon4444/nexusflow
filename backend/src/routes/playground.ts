...
router.post("/chat/completions", async (req: Request, res: Response) => {
  ...
  if (!session) {
    openAiError(res, 401, "Please log in before using the Playground.", "invalid_session");
    return;
  }
  ...
  if (!await hasSufficientBalance(session.id, estimatedChatCost)) {
    openAiError(res, 402, "Insufficient balance, please recharge before calling.", "insufficient_balance", "insufficient_balance");
    return;
  }
