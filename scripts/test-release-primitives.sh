...
case "$command" in
  ...
  jlist)
    ...
    if test -f "$NEXUSFLOW_TEST_STATE/app-quadrant-backend"; then
      backend='{"name":"quadrant-backend","pm2_env":{"status":"online","pm_cwd":"%s/backend","pm_exec_path":"%s/backend/dist/index.js","NODE_ENV":"%s","NEXUSFLOW_RELEASE_RUNTIME":"%s","ENABLE_MOCK_PAYMENT":"%s","ENABLE_SEED_API_KEYS":"%s","USE_PG_MEM":"%s","PROVIDER_OUTBOUND_HOST_ALLOWLIST":"api.anthropic.com,api.himodels.ai,developerhelena-1129-resource.services.ai.azure.com,dashscope.aliyuncs.com,app-api.pixverse.ai,ark.cn-beijing.volces.com,token.genvia.ai,jawayid.com","PROVIDER_OUTBOUND_ENDPOINT_ALLOWLIST":"jawayid.com:3000","PORT":3001,"BUILD_SHA":"%s"}}'
