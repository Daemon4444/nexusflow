...
function isTaskModelCategory(category?: string): boolean {
  return category === "Image Generation" || category === "Video Generation" || category === "Audio";
}
...
export default function AdminPage() {
    ...
    try {
      ...
      if (!providersRes.success || !modelsRes.success || !statsRes.success) {
        if (providersRes.status === 403 || providersRes.status === 401) {
          ...
          setError("Insufficient admin privileges or admin API not available");
        }
      }
    } catch (loadErr) {
  ...
  async function handleRejectProvider(id: string) {
    await withAction(`disable-provider-${id}`, async () => {
      const reason = prompt("Enter a note for disabling (optional):");
  ...
  async function handleRejectRequest(id: string) {
    await withAction(`reject-request-${id}`, async () => {
      const reply = prompt("Rejection reason (optional)", "Not approved at this time");
      const res = await fetchAPI(`/api/rate-limits/admin/requests/${id}/reject`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ reply: reply || "Not approved at this time" }),
      });
  ...
  async function handleCreateProvider() {
    ...
    const apiKey = prompt("API Key (leave blank to fill in later)", "") || "";
  ...
  async function handleSwitchChannel(providerId: string, channel: string) {
    ...
    if (res.success) {
      setNotice(res.message || "Channel switched");
      await loadData();
      await loadProviderDetail(providerId);
    } else {
      setNotice(res.message || "Channel switch failed");
    }
  }
  ...
  async function handleSaveCapacity(modelId: string, current?: CapacityRecord) {
    ...
    const enabled = prompt("Enabled: true / false", String(current?.isEnabled ?? true));
    ...
    if (res.success) {
      setNotice(`Capacity policy for model ${modelId} updated`);
      loadProviderDetail(selectedProviderId);
    }
  }

  async function handleSaveModelRoute(model: Model, route?: NonNullable<Model["routes"]>[number]) {
    const providerId = route?.providerId || prompt("Choose provider ID (visible in the provider console, e.g. dashscope or volcengine-ark)", providers[0]?.id || "");
    ...
    const priority = prompt("Priority (higher value = higher priority)", String(route?.priority ?? 0));
    if (priority === null) return;
    const weight = prompt("Weight (within the same priority, higher value is more likely to be selected)", String(route?.weight ?? 100));
    if (weight === null) return;
    const enabled = prompt("Enabled: true / false", String(route?.isEnabled ?? true));
    if (enabled === null) return;
    ...
    if (res.success) {
      setNotice(`Provider route for ${model.modelId} saved`);
      await loadData();
  ...
  async function submitDiscountForm(userId: string) {
    ...
    if (res.success) {
      ...
      setNotice(res.message || "Failed to save discount");
    }
  }
  ...
  async function handleDeleteUserDiscount(id: string) {
    await withAction(`delete-discount-${id}`, async () => {
      ...
      if (res.success) {
        ...
        setNotice(res.message || "Failed to delete discount");
      }
    });
  }
  ...
  async function submitBalanceForm(userId: string) {
    ...
    const description = balanceForm.description.trim() || (amountDelta > 0 ? "Admin balance increase" : "Admin balance deduction");
    ...
    if (res.success) {
      ...
      setNotice(res.message || "Failed to adjust balance");
    }
  }
  ...
  async function submitRateLimitForm(userId: string) {
    ...
    if (res.success) {
      ...
      setNotice(res.message || "Failed to update rate limit");
    }
  }
  ...
  async function handleDeleteUserRateLimit(userId: string, model: string) {
    await withAction(`delete-ratelimit-${userId}-${model}`, async () => {
      ...
      if (res.success) {
        ...
        setNotice(res.message || "Failed to delete rate limit");
      }
    });
  }
  ...
  async function handleExportUserBilling(userId: string) {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "Failed to export billing");
    }
  }
  ...
  async function submitDefaultLimitForm(userId: string) {
    ...
    if (res.success) {
      ...
      setNotice(res.message || "Failed to update default limits");
    }
  }
  ...
  const tabs: { key: TabKey; label: string }[] = [
    ...
    { key: "approvals", label: "Limit Approvals" },
    ...
    return (
      <div style={{ display: "flex", minHeight: "100vh", alignItems: "center", justifyContent: "center", background: "#f3f4f6", fontFamily: "system-ui, sans-serif" }}>
        <div style={{ textAlign: "center", padding: 40 }}>
          ...
          <div style={{ fontSize: 18, color: "#374151", marginTop: 8 }}>No admin privileges</div>
          ...
          <>
            ...
            {activeTab === "dashboard" ? (
              ...
              <>
                ...
                {stats ? (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 16, marginBottom: 20 }}>
                    {[
                      { label: "Providers Pending Setup", value: stats.providers.draft, color: "#d97706" },
                ...
                {monitorOverview ? (
                  <>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 16, marginBottom: 20 }}>
                      {[
                        { label: "Global Current RPM", value: monitorOverview.totals.currentRpm, color: "#2563eb" },
                        { label: "Global Current TPM", value: monitorOverview.totals.currentTpm, color: "#0f766e" },
                        { label: "Global Total RPM Limit", value: monitorOverview.totals.rpmLimit, color: "#7c3aed" },
                        { label: "Global Total TPM Limit", value: monitorOverview.totals.tpmLimit, color: "#b45309" },
                      ].map((item) => (
                        <div key={item.label} style={{ ...cardStyle, padding: 20 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 16, marginBottom: 20 }}>
                      {[
                        ...
                        { label: "Tripped Providers", value: monitorOverview.totals.downProviders, color: "#ef4444" },
                        { label: "Monitoring Time", value: new Date(monitorOverview.generatedAt).toLocaleTimeString("en-US"), color: "#2563eb" },
                      ].map((item) => (
                        <div key={item.label} style={{ ...cardStyle, padding: 20 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 16, marginBottom: 20 }}>
                      <div style={{ ...cardStyle, padding: 20 }}>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 14 }}>Provider Status</div>
                      ...
                      <div style={{ ...cardStyle, padding: 20 }}>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 14 }}>Alerts Panel</div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 }}>
                  ...
                  <div style={{ ...cardStyle, padding: 20 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: "#111827", marginBottom: 12 }}>Open Tickets</div>
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, fontSize: 13, color: "#6b7280" }}>Unified operations view across upstream providers, model routes, prices, rate limits, and health</div>
                  </div>
                ...
                {!operations ? (
                  <div style={{ ...cardStyle, padding: 48, textAlign: "center", color: "#6b7280" }}>No provider operations data</div>
                ) : (
                  <>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0, 1fr))", gap: 14, marginBottom: 18 }}>
                      {[
                        { label: "Enabled Providers", value: `${operations.summary.enabledProviders}/${operations.summary.providers}`, color: "#10b981" },
                        { label: "Routed Models", value: `${operations.summary.routedModels}/${operations.summary.models}`, color: "#2563eb" },
                        { label: "Enabled Routes", value: `${operations.summary.enabledRoutes}/${operations.summary.routes}`, color: "#0f766e" },
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: 16, marginBottom: 18, alignItems: "start" }}>
                      <div style={{ ...cardStyle, padding: 20 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 14 }}>
                          <h2 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: 0 }}>Upstream Provider Ledger</h2>
                          <span style={{ fontSize: 12, color: "#6b7280" }}>Generated at {new Date(operations.generatedAt).toLocaleTimeString("en-US")}</span>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div style={{ ...cardStyle, padding: 20 }}>
                        <h2 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: "0 0 14px" }}>Operational Risks</h2>
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          {operations.issues.length === 0 ? (
                            <div style={{ color: "#6b7280", fontSize: 13 }}>No configuration risks detected.</div>
                          ...
                          {operations.issues.length > 12 ? (
                            <div style={{ fontSize: 12, color: "#6b7280" }}>{operations.issues.length - 12} more risks; continue investigation in Model Management and the Provider Console.</div>
                          ) : null}
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18, alignItems: "start" }}>
                      <div style={{ ...cardStyle, padding: 20 }}>
                        <h2 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: "0 0 14px" }}>Customer / Model Routing Policies</h2>
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          {operations.routePolicies.length === 0 ? (
                            <div style={{ color: "#6b7280", fontSize: 13 }}>No customer-level overrides; the system schedules by global weight and health.</div>
                            ...
                            <div key={policy.id} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12 }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                                ...
                                <span style={{ fontSize: 12, color: policy.is_enabled ? "#059669" : "#6b7280" }}>{policy.is_enabled ? "Enabled" : "Disabled"}</span>
                              </div>
                      ...
                      <div style={{ ...cardStyle, padding: 20 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          {operations.routeAudits.length === 0 ? (
                            <div style={{ color: "#6b7280", fontSize: 13 }}>No route, cost, or policy changes recorded yet.</div>
                            ...
                            <div key={audit.id} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12 }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                                ...
                                <span style={{ fontSize: 12, color: "#6b7280" }}>{new Date(audit.created_at).toLocaleString("en-US")}</span>
                              </div>
                    ...
                    <div style={{ ...cardStyle, overflow: "hidden" }}>
                      <div style={{ padding: "16px 18px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", background: "#f8fafc" }}>
                        <div>
                          <h2 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: 0 }}>Model Routing Matrix</h2>
                          <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>Each row is one upstream-bearing route; prices come from the model catalog and capacity from provider configuration</div>
                        </div>
                              ...
                              return (
                                <tr key={`${route.providerId}-${route.modelId}`} style={{ background: route.enabled ? "#fff" : "#fafafa" }}>
                                  ...
                                  <td style={{ padding: "12px", borderBottom: "1px solid #f3f4f6", verticalAlign: "top" }}>
                                    ...
                                    <div style={{ marginTop: 5, color: route.recommended ? "#059669" : "#b45309" }}>{route.recommended ? "Recommended" : `Suggested: ${route.recommendedProviderId}`}</div>
                                  </td>
                                  <td style={{ padding: "12px", borderBottom: "1px solid #f3f4f6", verticalAlign: "top" }}>
                                    <span style={{ padding: "3px 8px", borderRadius: 9999, background: route.enabled ? "#ecfdf5" : "#f3f4f6", color: route.enabled ? "#166534" : "#6b7280" }}>{route.enabled ? "Enabled" : "Disabled"}</span>
                                  ...
                                  <td style={{ padding: "12px", borderBottom: "1px solid #f3f4f6", verticalAlign: "top", color: "#4b5563" }}>
                                    Sell input {route.promptPrice}
                                    <br />
                                    Sell output {route.completionPrice}
                                    <br />
                                  ...
                                  <td style={{ padding: "12px", borderBottom: "1px solid #f3f4f6", verticalAlign: "top", color: "#4b5563" }}>
                                    ...
                                    <br />
                                    {isTaskModel ? `Task concurrency ${route.concurrentLimit}` : `TPM ${route.currentTpm}/${route.tpmLimit}`}
                                    <br />
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, fontSize: 13, color: "#6b7280" }}>View accounts, balances, usage, default limits, and per-model overrides</div>
                  </div>
                  {usageOverview ? (
                  ...
                  <div style={{ ...cardStyle, overflow: "hidden" }}>
                    ...
                    <div style={{ padding: 12, borderBottom: "1px solid #e5e7eb", background: "#fff" }}>
                      <input
                        ...
                        placeholder="Search by nickname, email, phone, or user ID"
                    ...
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      {filteredUsers.length === 0 ? (
                        <div style={{ padding: 36, textAlign: "center", color: "#6b7280" }}>No users</div>
                      ...
                      <>
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start" }}>
                            <div>
                              ...
                              <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>Registered on {new Date(selectedUser.createdAt).toLocaleString("en-US")}</div>
                            </div>
                            <div style={{ textAlign: "right" }}>
                          ...
                          {balanceForm && (
                            <div style={{ marginTop: 14, padding: 14, background: "#f0fdf4", borderRadius: 10, border: "1px solid #bbf7d0" }}>
                              ...
                              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                                <button onClick={() => submitBalanceForm(selectedUser.id)} style={{ border: "none", background: "#111827", color: "#fff", borderRadius: 7, padding: "7px 16px", fontSize: 13, cursor: "pointer" }}>Confirm</button>
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                            <div>
                              ...
                              <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>Applies to all models without dedicated overrides</div>
                            </div>
                            {!defaultLimitForm && (
                              <button
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                            <h3 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: 0 }}>Per-Model Rate Limits</h3>
                            <button
                          ...
                          <div style={{ display: "flex", flexDirection: "column" }}>
                            {(() => {
                              ...
                              return limits.length > 0 ? limits.map((limit) => (
                                ...
                                <div style={{ padding: "12px 10px", color: "#6b7280", fontSize: 13 }}>No per-model overrides; default limits apply.</div>
                              );
                            })()}
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                            <div>
                              ...
                              <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>Override catalog price per user + model</div>
                            </div>
                            {!discountForm && (
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            {selectedUserDiscounts.length > 0 ? selectedUserDiscounts.map((discount) => (
                              <div key={discount.id} style={{ padding: 14, borderRadius: 10, border: "1px solid #e5e7eb" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                                  <div>
                                    ...
                                    <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>
                                      {discount.notes || "No notes"} · {discount.is_enabled ? "Enabled" : "Disabled"}
                                    </div>
                                  </div>
                                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                    <div style={{ textAlign: "right" }}>
                                      <div style={{ fontSize: 18, fontWeight: 700, color: "#111827" }}>{Math.round(Number(discount.discount_rate || 1) * 100)}%</div>
                                      <div style={{ fontSize: 11, color: "#6b7280" }}>Effective rate</div>
                                    </div>
                              ...
                              <div style={{ color: "#6b7280", fontSize: 13 }}>No model discounts configured; all models billed at catalog price.</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }}>
                              {(() => {
                                ...
                                return [
                                  ...
                                  { label: "Charged", value: `$${Number(usage.totalCost || 0).toFixed(4)}` },
                          ...
                          ) : <div style={{ color: "#6b7280", fontSize: 13 }}>No usage data</div>}
                        </div>
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                              ...
                              {selectedUserDetail.byModel.map((row) => (
                                <div key={row.model} style={{ display: "grid", gridTemplateColumns: "1fr 90px 110px 90px", gap: 10, padding: 12, border: "1px solid #e5e7eb", borderRadius: 10, alignItems: "center" }}>
                                  <div>
                                    ...
                                    <div style={{ marginTop: 3, fontSize: 11.5, color: "#6b7280" }}>{row.percentage}% of requests</div>
                                  </div>
                                  <div style={{ textAlign: "right", fontSize: 12, color: "#4b5563" }}>{row.requests.toLocaleString()} reqs</div>
                          ...
                          ) : <div style={{ color: "#6b7280", fontSize: 13 }}>No model usage details</div>}
                        </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                              {selectedUserDetail.recent.slice(0, 20).map((call, index) => (
                                <div key={`${call.time}-${call.model}-${index}`} style={{ display: "grid", gridTemplateColumns: "90px 1fr 96px 86px 70px", gap: 10, padding: 11, border: "1px solid #e5e7eb", borderRadius: 10, alignItems: "center" }}>
                                  ...
                                  <div style={{ textAlign: "right", fontSize: 12, color: call.status === "成功" || call.status === "success" ? "#059669" : "#dc2626" }}>{call.status}</div>
                                </div>
                              ))}
                            </div>
                          ) : <div style={{ color: "#6b7280", fontSize: 13 }}>No API call history</div>}
                        </div>
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          ...
                          ) : <div style={{ color: "#6b7280", fontSize: 13 }}>No transactions</div>}
                        </div>

                        {/* ── All requests history ── */}
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <h3 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: "0 0 14px" }}>Limit Request History</h3>
                          {userDetailLoading ? (
                                ...
                                <div key={req.id} style={{ padding: 14, borderRadius: 10, border: "1px solid #e5e7eb" }}>
                                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
                                    <div>
                                      ...
                                      {req.admin_reply ? <div style={{ marginTop: 6, fontSize: 12, color: "#334155", background: "#f8fafc", borderRadius: 6, padding: "6px 10px" }}>Admin note: {req.admin_reply}</div> : null}
                                    </div>
                                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                                      ...
                                      <span style={{ fontSize: 11, color: "#9ca3af" }}>{new Date(req.created_at).toLocaleString("en-US")}</span>
                                    </div>
                                  </div>
                                </div>
                              ))}
                            </div>
                          ) : <div style={{ color: "#6b7280", fontSize: 13 }}>No request history</div>}
                        </div>
                      </>
                    ) : (
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: "#111827", margin: 0 }}>Limit Approvals</h1>
                    <div style={{ marginTop: 6, fontSize: 13, color: "#6b7280" }}>Users submit QPM / TPM requests; approve or reject them here</div>
                  </div>
                  <button onClick={() => loadData()} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
                    Refresh
                  </button>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {rateLimitRequests.length === 0 ? (
                    <div style={{ ...cardStyle, padding: 48, textAlign: "center", color: "#6b7280" }}>No limit requests</div>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start" }}>
                          <div>
                            ...
                            <div style={{ marginTop: 8, display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12, color: "#4b5563" }}>
                              ...
                              <span>Created: {new Date(request.created_at).toLocaleString("en-US")}</span>
                            </div>
                            {request.reason ? (
                            ...
                            {request.admin_reply ? (
                              <div style={{ marginTop: 10, fontSize: 12.5, color: "#334155", background: "#f8fafc", borderRadius: 8, padding: 10 }}>
                                Admin note: {request.admin_reply}
                              </div>
                            ) : null}
                          </div>
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, fontSize: 13, color: "#6b7280" }}>Manage upstream provider configuration, capacity policies, and health</div>
                  </div>
                  <button onClick={() => loadData()} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
                    Refresh
                  </button>
                    ...
                    <div style={{ ...cardStyle, padding: 18 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {filteredProviders.length === 0 ? (
                          <div style={{ padding: 32, textAlign: "center", color: "#6b7280" }}>No providers</div>
                          ...
                          >
                            ...
                            <div style={{ fontSize: 12, color: "#6b7280", marginTop: 10 }}>
                              Models {provider.modelCount ?? 0} · Routes {provider.enabledRoutes ?? 0} · TPM {Number(provider.currentTpm || 0).toLocaleString()}/{Number(provider.tpmLimit || 0).toLocaleString()}
                            </div>
                          </button>
                        ))}
                      </div>
                      ...
                      <>
                        <div style={{ ...cardStyle, padding: 20 }}>
                          ...
                          {selectedProvider.channelConfig ? (
                            <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 14, marginBottom: 16, background: "#f8fafc" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", marginBottom: 12 }}>
                                <div>
                                  <div style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>Active Sub-Channel</div>
                                  <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
                            <div style={{ fontSize: 12, color: "#6b7280" }}>
                              {selectedProvider.createdAt ? `Created on ${new Date(selectedProvider.createdAt).toLocaleString("en-US")}` : "Built-in upstream provider"}
                            </div>
                            <button onClick={handleSaveProvider} disabled={savingProvider} style={{ padding: "10px 18px", background: "#111827", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", opacity: savingProvider ? 0.55 : 1 }}>
                              {savingProvider ? "Saving..." : "Save Provider Configuration"}
                            </button>
                          </div>
                        </div>
                          ...
                          <div style={{ ...cardStyle, padding: 20 }}>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }}>
                              {[
                                ...
                                { label: "Enabled Routes", value: selectedProviderTotals.enabledRoutes },
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                            <h3 style={{ fontSize: 17, fontWeight: 700, color: "#111827", margin: 0 }}>Capacity & Routing</h3>
                            <div style={{ fontSize: 12, color: "#6b7280" }}>Text/VL: track RPM/TPM. Image/Video/Audio: track task submission rate and background concurrency</div>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            {providerDetail.models.length === 0 ? (
                              <div style={{ color: "#6b7280", fontSize: 13 }}>This provider has no models yet.</div>
                                  ...
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                                    <div>
                                      ...
                                      <div style={{ marginTop: 8, display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12, color: "#6b7280" }}>
                                        ...
                                        {isTaskModel ? (
                                          <span>Task Concurrency: {capacity?.concurrentLimit ?? 10}</span>
                                        ) : (
                        ...
                        <div style={{ ...cardStyle, padding: 20 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                            ...
                            <div style={{ fontSize: 12, color: "#6b7280" }}>Capacity hit rate, circuit breaker state, latency trends</div>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            {providerRouteMetrics.length === 0 ? (
                              <div style={{ color: "#6b7280", fontSize: 13 }}>No route monitoring data yet.</div>
                              ...
                              <div key={route.modelId} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 14 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                                  <div>
                                    ...
                                    <div style={{ marginTop: 8, display: "flex", gap: 12, flexWrap: "wrap", fontSize: 12, color: "#6b7280" }}>
                                      ...
                                      {isTaskModel ? <span>Task Concurrency {route.concurrentLimit}</span> : <span>TPM {route.currentTpm}/{route.tpmLimit}</span>}
                                      <span>Latency {route.avgLatencyMs} ms</span>
                                      <span>Failures {route.consecutiveFailures}</span>
                                ...
                                {route.lastError ? (
                                  <div style={{ marginTop: 10, fontSize: 12, color: "#b91c1c" }}>Last error: {route.lastError}</div>
                                ) : null}
                              </div>
                              );
                            })}
                ...
                <>
                  <div style={{ ...cardStyle, padding: 16, marginBottom: 16 }}>
                    ...
                    <div style={{ marginTop: 10, fontSize: 12, color: "#6b7280" }}>{filteredByModelModels.length} models</div>
                  </div>
                      ...
                      <div key={model.id} style={{ ...cardStyle, padding: "16px 20px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                          ...
                          <div style={{ fontSize: 12, color: "#6b7280", whiteSpace: "nowrap" }}>
                            {model.routes?.filter((r) => r.isEnabled).length || 0}/{model.routes?.length || 0} routes enabled
                          </div>
                        </div>
                            ...
                            return (
                              ...
                              >
                                <div>
                                  ...
                                  <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>
                                    <span style={{ color: route.isEnabled ? "#10b981" : "#ef4444", fontWeight: 600 }}>{route.isEnabled ? "Enabled" : "Disabled"}</span>
                                  </div>
                                </div>
                                {isTask ? (
                                  <div style={{ fontSize: 12, color: "#4b5563" }}>
                                    RPM {route.currentRpm}/{route.rpmLimit}<br />
                                    Task concurrency {route.concurrentLimit}
                                  </div>
                                ) : (
                                  <div style={{ fontSize: 12, color: "#4b5563" }}>
                                    RPM {route.currentRpm}/{route.rpmLimit}<br />
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, fontSize: 13, color: "#6b7280" }}>Manage upstream provider routes per model. e.g. glm-5.1 can be served from both DashScope and Ark, with priority/weight controlling selection.</div>
                  </div>
                  <button onClick={() => loadData()} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
                    Refresh
                  </button>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {filteredModels.length === 0 ? (
                    <div style={{ ...cardStyle, padding: 48, textAlign: "center", color: "#6b7280" }}>No models</div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                        ...
                        {model.status === "draft" ? (
                          <div style={{ display: "flex", gap: 8 }}>
                            ...
                            >
                              {isActing(`disable-model-${model.id}`) ? "..." : "Disable"}
                            </button>
                          </div>
                        ) : null}
                      </div>
                      ...
                      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
                        {model.routes && model.routes.length > 0 ? model.routes.map((route) => {
                          ...
                          <div key={`${model.modelId}-${route.providerId}`} style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 0.8fr auto", gap: 12, alignItems: "center", padding: 12, borderRadius: 10, border: "1px solid #e5e7eb", background: route.isEnabled ? "#f8fafc" : "#fff" }}>
                            <div>
                              <div style={{ fontSize: 13, fontWeight: 700, color: "#111827" }}>{route.providerName}</div>
                              <div style={{ marginTop: 4, fontSize: 12, color: "#6b7280" }}>{route.providerId} · {route.isEnabled ? "Enabled" : "Disabled"}</div>
                            </div>
                            {isTaskModel ? (
                              <div style={{ fontSize: 12, color: "#4b5563" }}>
                          ...
                          <div style={{ fontSize: 12.5, color: "#b91c1c", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: 12 }}>
                            No provider routes enabled; API calls will not select this model.
                          </div>
                        )}
              ...
              <>
                <h1 style={{ fontSize: 24, fontWeight: 700, color: "#111827", marginBottom: 20 }}>Tickets</h1>
                <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <>
                ...
                <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 12, padding: "8px 12px", background: "#f8fafc", borderRadius: 6, border: "1px solid #e5e7eb" }}>
                  Tip: Admins can view logs from all users. Enter a Request ID for an exact match. Detailed log data may take 1-2 minutes to become available after the request.
                </div>
                        ...
                        {logExpandedId === r.log_id && (
                          <div style={{ padding: "16px 20px", background: "#f9fafb", borderTop: "1px solid #e5e7eb", borderBottom: "1px solid #e5e7eb" }}>
                            {logDetailLoading ? (
                              ...
                              <div style={{ color: "#6b7280", fontSize: 13 }}>{logDetailNote || "No detail data"}</div>
                            )}
                          </div>
                        )}
                      </div>
