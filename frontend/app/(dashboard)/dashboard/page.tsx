    ...
    <UserLayout wide>
      <div className="quiet-console-page">
        ...
        {loading ? (
          ...
          <ErrorState title="Failed to load console" message={error} onAction={loadDashboard} />
        ) : (
          <>
            ...
            <section className="quiet-primary-panel">
              <div className="quiet-chart-panel">
                ...
                {daily.length === 0 ? (
                  <EmptyState compact title="No usage trend yet" message="After your first API call, real requests and costs will appear here." />
                ) : (
            ...
            <section className="quiet-data-panel">
              <div className="quiet-data-half">
                ...
                {recent.length === 0 ? (
                  <EmptyState compact title="No request history yet" message="After your first request, the latest status will appear here." />
                ) : (
                      ...
                      <tbody>
                        {recent.slice(0, 6).map((item, index) => (
                          <tr key={`${item.time}-${item.model}-${index}`}>
                            ...
                            <td><span className={item.status === "success" ? "quiet-status success" : "quiet-status error"}><i />{item.status}</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="quiet-data-half">
                ...
                {recommendedModels.length === 0 ? (
                  <EmptyState compact title="Model catalog unavailable" />
                ) : (
                  <div className="quiet-table-scroll">
                    <table>
                      <thead>
