...
const tabs: { key: TabKey; label: string; model: string }[] = [
  ...
  { key: "edit", label: "Video Editing", model: "happyhorse-1.0-video-edit" },
];
...
const requestParams: Record<TabKey, { name: string; type: string; required: boolean; desc: string }[]> = {
  t2v: [
    ...
    { name: "prompt", type: "string", required: true, desc: "Text prompt describing the video to generate. Supports English and Chinese, up to 2500 Chinese characters." },
    { name: "resolution", type: "string", required: false, desc: "Resolution tier: 720P (default) or 1080P. Affects billing: 720P $0.9/sec, 1080P $1.6/sec." },
    { name: "ratio", type: "string", required: false, desc: "Aspect ratio. Options: 16:9 (default), 9:16, 1:1, 4:3, 3:4." },
    { name: "duration", type: "integer", required: false, desc: "Video duration in seconds, range [3, 15], default 5." },
  ...
  i2v: [
    ...
    { name: "img_url", type: "string", required: true, desc: "First-frame reference image URL (JPG/PNG/WEBP, ≤10MB)." },
  ...
  r2v: [
    ...
    { name: "img_urls", type: "string[]", required: true, desc: "Array of reference image URLs, 1-9 images." },
    { name: "resolution", type: "string", required: false, desc: "Resolution tier: 720P (default) or 1080P." },
  ...
  edit: [
    ...
    { name: "prompt", type: "string", required: true, desc: "Edit instruction describing the changes to make to the video." },
    { name: "video_url", type: "string", required: true, desc: "Input video URL (3-60 sec; anything over 15 sec is truncated)." },
...
const curlExamples: Record<TabKey, string> = {
  ...
  -d '{
    ...
    "prompt": "Replace the background with a snowy mountain scene while keeping the character's movements unchanged",
    "resolution": "720P"
  }'`,
};
...
response = requests.post(
    ...
    json={
        "model": "happyhorse-1.0-i2v",
        "img_url": "https://example.com/first-frame.jpg",
        "prompt": "The character slowly turns their head, hair blown by the sea breeze",
        "resolution": "720P",
        "duration": 8,
    },
).json()
...
# Step 1: Create the reference-to-video task (1-9 reference images)
response = requests.post(
    f"{BASE}/v1/tasks",
    headers={
        "Authorization": f"Bearer {API_KEY}",
    ...
    json={
        "model": "happyhorse-1.0-r2v",
        "prompt": "A man sits in a chair by the window playing guitar",
        "img_urls": [
            "https://example.com/ref-person.jpg",
            "https://example.com/ref-scene.jpg",
        ],
...
# Step 1: Create the video editing task
response = requests.post(
    f"{BASE}/v1/tasks",
    headers={
        "Authorization": f"Bearer {API_KEY}",
...
export default function HappyHorseApiPage() {
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <div style={{
          ...
          <ul style={{ margin: "8px 0 0", paddingLeft: 18 }}>
            ...
            <li>Poll at a recommended interval of <strong>10-15 seconds</strong>.</li>
  ...
  "completed_at": "2026-06-01T10:00:05.000Z"
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              <tr style={{ background: "var(--bg)" }}>
                ...
                <td style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", color: "var(--text-secondary)" }}>Billed by output video duration (e.g. 5 sec = $4.5)</td>
              </tr>
              <tr style={{ background: "var(--bg-elevated)" }}>
                ...
                <td style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", color: "var(--text-secondary)" }}>Billed by output video duration (e.g. 5 sec = $8.0)</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {[
          { href: "/docs/models/happyhorse", label: "HappyHorse Page", desc: "Model updates and capability details" },
