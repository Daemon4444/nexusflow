    ...
    <div style={{ padding: "48px 64px", maxWidth: 1000 }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 15, color: "var(--text-secondary)", lineHeight: 1.8, maxWidth: 720, margin: 0 }}>
          A compatibility entry point that accepts the Google Gemini GenerateContent request/response format, suitable for migrating an existing Gemini SDK or HTTP client to NexusFlow.
          This is not Google native Gemini model hosting—the <code>model</code> in the path must be a NexusFlow model ID such as <code>qwen-turbo</code>.
        </p>
      </div>
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {[
                ["model", true, "Path parameter. Must be a NexusFlow model ID such as qwen-turbo; do not pass a Google native model name."],
                ["key", true, "API key. Recommended in the query string: ?key=sk-air-...; also accepts Authorization: Bearer."],
                ["contents", true, "Array of conversation contents; each item has a role (user/model) and parts (array of text)."],
                ["generationConfig.temperature", false, "Sampling temperature in [0, 2]. Default 1.0."],
                ["generationConfig.maxOutputTokens", false, "Maximum number of output tokens."],
                ["generationConfig.topP", false, "Top-P sampling."],
                ["generationConfig.topK", false, "Top-K sampling."],
                ["generationConfig.stopSequences", false, "Array of stop sequences."],
                ["tools", false, "Array of tool/function definitions (function calling)."],
    ...
    "totalTokenCount": 268
      ...
      <section style={{ marginBottom: 36 }}>
        <div style={{
          ...
          <strong>Note:</strong> The Gemini compatibility layer automatically converts requests to the platform's internal format, routes them to the corresponding model, and converts the response back to Gemini format.
          This is not the Google official Gemini backend, and it does not guarantee that Google native model names work; model capabilities follow the NexusFlow model list.
          If you don't depend on a Gemini SDK, we recommend using <code>/v1/chat/completions</code> (OpenAI format) directly for the most complete feature support.
        </div>
      </section>

      <section style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {[
          { href: "/docs/api/chat", label: "Chat Completions", desc: "OpenAI-format chat endpoint" },
