...
const requestParams: Record<TabKey, { name: string; type: string; required: boolean; desc: string }[]> = {
  image: [
    { name: "model", type: "string", required: true, desc: "Model ID, e.g. wan2.6-t2i. See the model docs for the full list." },
    { name: "prompt", type: "string", required: true, desc: "Text prompt describing the image you want to generate." },
    { name: "negative_prompt", type: "string", required: false, desc: "Negative prompt describing what you do not want to appear." },
    { name: "size", type: "string", required: false, desc: "Output image size, e.g. 1024x1024 (default), 720x1280, 1280x720, etc." },
    { name: "n", type: "integer", required: false, desc: "Number of images to generate, default 1, max 4." },
    { name: "seed", type: "integer", required: false, desc: "Random seed; fixing the seed improves reproducibility." },
  ],
  video: [
    { name: "model", type: "string", required: true, desc: "Model ID, e.g. happyhorse-1.0-t2v. See the model docs for the full list." },
    { name: "prompt", type: "string", required: true, desc: "Text prompt describing the video you want to generate. Supports English and Chinese." },
    { name: "resolution", type: "string", required: false, desc: "Resolution tier: 720P (default) or 1080P. Affects billing." },
    { name: "ratio", type: "string", required: false, desc: "Aspect ratio, e.g. 16:9 (default), 9:16, 1:1, 4:3, 3:4." },
    { name: "duration", type: "integer", required: false, desc: "Video duration in seconds; ranges vary by model, default 5." },
    { name: "img_url", type: "string", required: false, desc: "First-frame reference image URL (required for image-to-video models)." },
    { name: "watermark", type: "boolean", required: false, desc: "Whether to add a watermark, default true." },
    { name: "seed", type: "integer", required: false, desc: "Random seed [0, 2147483647]." },
  ],
};
...
const curlExamples: Record<TabKey, string> = {
  ...
  -d '{
    ...
    "prompt": "A miniature city built from cardboard and bottle caps comes alive at night. A cardboard train slowly rolls past.",
    "resolution": "1080P",
    "ratio": "16:9",
    "duration": 5
  }'`,
...
# wan2.6-t2i is a synchronous endpoint that returns results directly
...
export default function TasksApiPage() {
    ...
    <div style={{ padding: "48px 64px", maxWidth: 1000 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 15, color: "var(--text-secondary)", lineHeight: 1.8, maxWidth: 720, margin: 0 }}>
          Image and video generation both go through the <code>/v1/tasks</code> endpoint. Video models are asynchronous (create a task to get a task_id, then poll for the result), while image models (such as wan2.6-t2i) are synchronous and return results directly without polling. Video generation usually takes 1-5 minutes.
        </p>
      </div>
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <div style={{
          ...
          <ul style={{ margin: "8px 0 0", paddingLeft: 18 }}>
            <li>After successful creation, query the result using the returned <code>id</code> (the task_id). <strong>Do not recreate the task</strong> — just poll.</li>
            <li><strong>Image models</strong> (such as wan2.6-t2i) are synchronous: the create request returns results directly, no polling needed.</li>
            <li>For <strong>video models</strong>, poll at a recommended interval of <strong>10-15 seconds</strong>.</li>
            <li>task_id queries are valid for <strong>24 hours</strong>; after that they cannot be queried.</li>
            <li>Output file URLs are valid for 24 hours; download and save them immediately.</li>
            <li>Only image and video models support the <code>/v1/tasks</code> endpoint; use <code>/v1/chat/completions</code> for chat models.</li>
          </ul>
        </div>
      </section>
      ...
      {/* Step 1: Create task */}
      <section style={{ marginBottom: 36 }}>
        <h2 style={{ fontSize: 20, fontWeight: 600, color: "var(--text-primary)", marginBottom: 16 }}>Step 1: Create a Task</h2>
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.7, marginBottom: 14 }}>
          On success, task information is returned (HTTP 202); the <code>id</code> is the task_id used for subsequent polling.
        </p>
  ...
  "created_at": "2026-06-01T10:00:00.000Z"
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {[
                ["id", "Task ID (task_id). Used to poll for video task results, valid for 24 hours."],
                ["object", "Always \"task\"."],
                ["status", "Video models start as running; image models (such as wan2.6-t2i) are synchronous and return succeeded directly."],
  ...
  "completed_at": "2026-06-01T10:00:05.000Z"
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {[
                ...
                ["output", "object", "Generation result (returned only when succeeded). Image tasks include image_url, video tasks include video_url. Links are valid for 24 hours."],
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.7, marginBottom: 14 }}>
          Retrieve the current user's recent tasks. Use the <code>limit</code> parameter to control the number returned (default 20, max 100).
        </p>
  ...
  "data": [
    ...
    },
    {
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
          {[
            ...
            { title: "Separate sync and async", desc: "Route chat requests through /v1/chat/completions and image/video through /v1/tasks to reduce interference." },
            { title: "Download results promptly", desc: "Output URLs are valid for 24 hours; download and save files immediately after a task completes." },
            { title: "Handle failure retries", desc: "When a task fails, check the error field. Fix parameter errors before retrying; for upstream timeouts, simply recreate the task." },
          ].map((item) => (
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {[
          { href: "/docs/api/happyhorse", label: "HappyHorse API", desc: "View the dedicated HappyHorse video generation docs" },
          { href: "/docs/api/limits", label: "Rate Limits", desc: "Limits and optimization tips for high concurrency" },
