...
const requestParams: { name: string; type: string; required: boolean; default?: string; desc: string; link?: string }[] = [
  { name: "model", type: "string", required: true, desc: "Model ID. For example qwen3.5-plus, deepseek-v4-flash, etc.", link: "/docs/models" },
  { name: "messages", type: "array", required: true, desc: "Array of conversation messages. Each message has a role (system / user / assistant / tool) and a content field. content can be a string or an array of content blocks; multimodal content availability depends on the model's capabilities." },
  { name: "stream", type: "boolean", required: false, default: "false", desc: "Whether to enable streaming output. When enabled, tokens are returned incrementally as SSE (Server-Sent Events)." },
  { name: "temperature", type: "number", required: false, default: "1.0", desc: "Sampling temperature, range [0, 2). Higher values are more random, lower values more deterministic. Adjust either this or top_p, not both." },
  { name: "top_p", type: "number", required: false, default: "1.0", desc: "Nucleus sampling probability threshold, range (0, 1]. The model samples only from the smallest token set whose cumulative probability reaches top_p." },
  { name: "max_tokens", type: "integer", required: false, desc: "Maximum number of tokens to generate. Limits vary by model; the model default is used when not set." },
  { name: "tools", type: "array", required: false, desc: "List of available tool/function definitions for function calling. Each tool has a type and a function field." },
  { name: "tool_choice", type: "string | object", required: false, default: '"auto"', desc: 'Tool selection strategy. Stably supports "auto", "none", or {"type":"function","function":{"name":"..."}} to specify a function. Forcing a tool is not recommended for thinking mode models.' },
  { name: "stop", type: "string | string[]", required: false, desc: "A stop word or array of stop words (up to 4). The model ends output immediately when it generates a stop word." },
  { name: "frequency_penalty", type: "number", required: false, default: "0", desc: "Frequency penalty, range [-2.0, 2.0]. Positive values penalize tokens by how often they have appeared, reducing repetition." },
  { name: "presence_penalty", type: "number", required: false, default: "0", desc: "Presence penalty, range [-2.0, 2.0]. Positive values penalize tokens that have already appeared, increasing topic diversity." },
  { name: "enable_thinking", type: "boolean", required: false, desc: "Whether to enable thinking mode. Only hybrid thinking models support the true/false toggle; thinking-only models keep thinking even if false is passed." },
...
const responseFields: { name: string; type: string; desc: string }[] = [
  ...
  { name: "created", type: "integer", desc: "Creation time as a Unix timestamp (seconds)." },
  { name: "model", type: "string", desc: "The model name actually used." },
  { name: "choices", type: "array", desc: "Array of generated results (usually one element)." },
  { name: "choices[].index", type: "integer", desc: "Index of the result within the array." },
  { name: "choices[].message.role", type: "string", desc: 'Message role, always "assistant".' },
  { name: "choices[].message.content", type: "string | null", desc: "Generated text content. May be null when the model calls a tool." },
  { name: "choices[].message.reasoning_content", type: "string", desc: "Chain-of-thought content returned by reasoning models (e.g. QwQ). Non-reasoning models do not return this field." },
  { name: "choices[].message.tool_calls", type: "array", desc: "Array of tool call requests. Returned only when the model decides to call a tool." },
  { name: "choices[].finish_reason", type: "string", desc: 'Stop reason: stop (natural end), length (reached max_tokens), tool_calls (called a tool).' },
  { name: "usage.prompt_tokens", type: "integer", desc: "Number of tokens consumed by the input." },
  { name: "usage.completion_tokens", type: "integer", desc: "Number of tokens consumed by the output." },
  { name: "usage.total_tokens", type: "integer", desc: "Total tokens consumed (prompt_tokens + completion_tokens)." },
];
...
const streamFields: { name: string; type: string; desc: string }[] = [
  ...
  { name: "choices[].delta.role", type: "string", desc: 'Appears only in the first chunk, with value "assistant".' },
  { name: "choices[].delta.content", type: "string", desc: "Incremental text content of this chunk." },
  { name: "choices[].delta.reasoning_content", type: "string", desc: "Incremental chain-of-thought content of this chunk (reasoning models)." },
  { name: "choices[].delta.tool_calls", type: "array", desc: "Incremental tool call data (streaming function calling)." },
  { name: "choices[].finish_reason", type: "string | null", desc: "Non-null only in the final chunk, indicating the stop reason." },
  { name: "usage", type: "object", desc: 'Returned in the final chunk only when stream_options.include_usage is true, reporting token usage.' },
];
...
# data: {"id":"...","choices":[{"delta":{"content":"Spr"},...}]}
# data: [DONE]`,
    python: `from openai import OpenAI
...
for await (const chunk of stream) {
  ...
  -d '{
    ...
    "messages": [
      ...
      {"role": "user", "content": "How do I handle character encoding issues?"}
    ]
  }'`,
    python: `from openai import OpenAI
...
conversation.append({"role": "user", "content": "How do I handle character encoding issues?"})
...
conversation.push({ role: "user", content: "How do I handle character encoding issues?" });
    ...
    <div style={{ padding: "48px 64px", maxWidth: 1000 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 15, color: "var(--text-secondary)", lineHeight: 1.8, maxWidth: 720, margin: 0 }}>
          Creates a chat completion response. The endpoint is fully compatible with the OpenAI Chat Completions format and can be used directly with the official OpenAI SDKs (Python / Node.js) — just change <code style={{ fontSize: 13, background: "var(--bg-elevated)", padding: "2px 6px", borderRadius: 4 }}>base_url</code> and <code style={{ fontSize: 13, background: "var(--bg-elevated)", padding: "2px 6px", borderRadius: 4 }}>api_key</code>. Supports streaming, multi-turn conversations, function calling, vision, and more.
        </p>
      </div>
      ...
      <section style={{ marginBottom: 32 }}>
        <div style={{
          ...
          <div style={{ fontSize: 13, color: "#065f46", lineHeight: 1.7 }}>
            <code style={{ fontSize: 12, fontWeight: 700 }}>/v1/chat/completions</code> supports all models available on NexusFlow — including Qwen, GLM, DeepSeek, Kimi, MiniMax, and more.
          </div>
        </div>
      </section>
...
data: [DONE]`} />
        ...
        <div style={{
          ...
          <strong>Tip:</strong> When using the OpenAI SDK, you do not need to parse SSE manually — the SDK handles streaming responses and provides an iterator interface. You only need to parse SSE data yourself when using cURL or a raw HTTP client.
        </div>
      </section>

      {/* ───────── Notes ───────── */}
      <section style={{ marginBottom: 36 }}>
        <h2 style={sectionHeading}>Protocol Pass-through</h2>
        <div style={{
          ...
          NexusFlow's <code>/v1/chat/completions</code> is fully pass-through with the OpenAI Chat Completions protocol:
          the request body is forwarded to the upstream as-is, and the response is returned as-is. Extension fields such as <code>tools</code>, <code>tool_choice</code>, <code>response_format</code>,
          <code>enable_thinking</code>, <code>thinking_budget</code>, <code>enable_search</code>, <code>search_options</code>,
          <code>seed</code>, <code>top_k</code>, <code>logprobs</code>, and <code>stream_options</code> can be used directly.
          Actual support depends on the model.
        </div>
      </section>

      {/* ───────── Pricing Info ───────── */}
      <section style={{ marginBottom: 36 }}>
        <h2 style={sectionHeading}>Billing</h2>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, lineHeight: 1.7, color: "var(--text-secondary)", marginBottom: 12 }}>
            Model families such as Qwen and GLM use <strong>tiered pricing based on the request's input token count</strong>. The total prompt tokens of a single request determine the applicable price tier, and input and output are billed at that tier's unit prices respectively.
          </p>
          ...
          <p style={{ fontSize: 12, color: "var(--text-tertiary)", lineHeight: 1.6 }}>
            For example, a request with 50K input tokens + 2K output tokens is billed at $4/M for input and $16/M for output (Tier 2). See the full tiered pricing on the <Link href="/pricing" style={{ color: "#1d4ed8" }}>Pricing page</Link>.
          </p>
        </div>

        <div>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)", marginBottom: 10 }}>Context Cache (Prompt Caching)</h3>
          <p style={{ fontSize: 13, lineHeight: 1.7, color: "var(--text-secondary)", marginBottom: 12 }}>
            Context caching is supported when calling via <code>/v1/messages</code> (Anthropic protocol). For repeated system prompts or long documents, the upstream automatically caches the prompt prefix, and subsequent requests get a discount on the cached portion:
          </p>
          <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden", fontSize: 13 }}>
            ...
            {[
              ["cache_creation_input_tokens", "1.25x input price", "First write to cache, slightly higher than regular input"],
              ["cache_read_input_tokens", "0.1x input price", "Cache hit, 90% discount"],
              ["input_tokens (non-cached)", "1x input price", "Billed normally"],
          ...
          <p style={{ fontSize: 12, color: "var(--text-tertiary)", lineHeight: 1.6, marginTop: 10 }}>
            <code>/v1/chat/completions</code> supports explicit caching via the <code>enable_context_caching: true</code> parameter. <code>/v1/messages</code> (Anthropic protocol) supports the <code>cache_control</code> content block annotation. Both protocols automatically benefit from implicit cache discounts.
          </p>
        </div>
      </section>
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <div style={{
          ...
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            <li>The <code>max_tokens</code> limit varies by model; see the <Link href="/docs/models" style={{ color: "#1d4ed8" }}>Models</Link> list for each model's limits.</li>
            <li>Adjust only one of <code>temperature</code> or <code>top_p</code>; setting both may produce unpredictable results.</li>
            <li>In streaming output, only the final chunk's <code>finish_reason</code> is non-null, marking the end of generation.</li>
            <li>For image understanding, use multimodal models such as the Qwen-VL series. <code>content</code> must be an array that includes an <code>image_url</code> type.</li>
            <li>For function calling, use model families that support tools, such as Qwen, DeepSeek, and GLM.</li>
            <li>Thinking mode (<code>enable_thinking</code>) must be used per model ID; see the support matrix in the <Link href="/docs/api/parameters" style={{ color: "#1d4ed8" }}>Parameter Matrix</Link>.</li>
            <li>The request body is passed through to the upstream protocol; extension fields beyond this doc (such as <code>thinking_budget</code>, <code>enable_search</code>, <code>search_options</code>) can be used directly, with actual support depending on the model.</li>
            <li>For the full parameter reference and model compatibility matrix, see the <Link href="/docs/api/parameters" style={{ color: "#1d4ed8" }}>Parameter Matrix</Link>.</li>
          </ul>
        </div>
      </section>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {[
          { href: "/docs/models", label: "Models", desc: "Browse all available models and capabilities" },
          { href: "/docs/api/errors", label: "Error Codes", desc: "Error code reference and troubleshooting guide" },
