...
const models = [
  { id: "deepseek-v4-pro", category: "Reasoning Model", context: "1M", input: 12, output: 24, desc: "V4 flagship reasoning model" },
  { id: "deepseek-v4-flash", category: "LLM", context: "1M", input: 1, output: 2, desc: "V4 high-speed chat model" },
  { id: "deepseek-v3.2", category: "LLM", context: "128K", input: 2, output: 3, desc: "V3.2 general-purpose model" },
  { id: "deepseek-r1", category: "Reasoning Model", context: "128K", input: 4, output: 16, desc: "R1 reasoning model" },
  { id: "deepseek-v3", category: "LLM", context: "128K", input: 2, output: 8, desc: "V3 general-purpose model" },
];
...
const curlExamples: Record<TabKey, string> = {
  ...
  -d '{
    "model": "deepseek-r1",
    "messages": [
      {"role": "user", "content": "A pool has two inlet pipes and one outlet pipe. Inlet A adds 3 tons of water per hour, inlet B adds 2 tons per hour, and the outlet drains 1.5 tons per hour. The pool holds 20 tons. Starting from empty, how long does it take to fill?"}
    ],
    "stream": true,
    ...
    <div style={{ padding: "48px 64px", maxWidth: 1000 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 15, color: "var(--text-secondary)", lineHeight: 1.8, maxWidth: 720, margin: 0 }}>
          The DeepSeek model family supports reasoning mode and streaming output. Text DeepSeek models can be called via three public protocols: OpenAI Chat Completions, Anthropic Messages, and Responses API.
        </p>
      </div>
      ...
      {/* Reasoning mode */}
      <section style={{ marginBottom: 36 }}>
        <h2 style={{ fontSize: 20, fontWeight: 600, color: "var(--text-primary)", marginBottom: 14 }}>Reasoning Mode</h2>
        <div style={{
          ...
          DeepSeek R1 is a thinking-only model and still returns reasoning_content even when <code>enable_thinking=false</code> is passed.
          DeepSeek V3.2 and V4 Pro are verified hybrid thinking models that can be controlled with the <code>enable_thinking</code> toggle.
        </div>
        <div style={{ background: "#111827", borderRadius: 8, padding: 18, overflow: "auto" }}>
          <DocsCodeBlock code={`// In reasoning mode responses, the delta includes a reasoning_content field
{
  "choices": [{
    "delta": {
...
// After thinking completes, it switches to the final answer
{
  "choices": [{
    "delta": {
      "reasoning_content": "",
