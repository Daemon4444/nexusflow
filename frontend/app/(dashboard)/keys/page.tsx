  ...
  async function loadKeys(signal?: AbortSignal) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || "API Key load failed");
      }
    } catch {
      setError("Unable to connect to Key service, please try again later");
    } finally {
  ...
  return (
    <UserLayout>
        ...
        <div className="usr-section animate-fadeIn" style={{ marginBottom: 20, borderColor: "rgba(16,185,129,0.3)" }}>
          ...
          <div className="usr-section-body">
            <p style={{ margin: "0 0 10px", fontSize: 13, color: "var(--text-secondary)" }}>
              Copy and save immediately. For security, only a masked key will be shown after closing.
            </p>
            ...
            <div className="key-next-call">
              <div className="key-next-call-head">
                <strong>Next step: Copy the request and make your first call</strong>
      ...
      {dataLoading ? (
        ...
        <ErrorState title="API Key load failed" message={error} onAction={loadKeys} compact />
      ) : keys.length === 0 ? (
          ...
          {keys.map((key) => (
            <div key={key.id} className="usr-section animate-fadeIn">
              ...
              <div style={{
                ...
                <span style={{ flexShrink: 0, fontSize: 12, color: "#a8a29e" }}>Full key only visible at creation</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {!createdKey && keys.length > 0 && (
        <div className="usr-section">
          <div className="usr-section-header">
            <div>
              <h3>Call Examples</h3>
              <p>Keys are fully visible only at creation; placeholders are used here to show the production request structure.</p>
            </div>
      ...
      {deleteTarget && (
        <div className="modal-backdrop" role="presentation">
          <div className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-key-title">
            ...
            <p>Are you sure you want to delete "{deleteTarget.name}"? Requests using this key will fail immediately after deletion.</p>
