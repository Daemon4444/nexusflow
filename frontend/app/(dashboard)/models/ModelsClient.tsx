...
const categoryColors: Record<string, string> = {
  "Large Language Model": "#2563eb", "Reasoning Model": "#dc2626",
  "Multimodal": "#7c3aed", "Multimodal Model": "#7c3aed",
  "Coding Model": "#0891b2", "Image Generation": "#db2777", "Video Generation": "#f97316",
  "Embedding": "#0f766e", "Embedding Model": "#0f766e",
  "Specialized": "#64748b", "Specialized Model": "#64748b",
  "Audio": "#7c2d12", "Audio Model": "#7c2d12",
};
  ...
  async function loadModels(signal?: AbortSignal) {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Only refresh global categories when no filter is applied — prevents the category buttons from disappearing
        if (!selectedProvider && selectedCategory === "All" && !search) {
          setAllCategories(res.categories || []);
        }
      } else {
        setModels([]);
        setError(res.message || "Model service is temporarily unavailable. Please try again later.");
      }
    } catch (err) {
      if (err instanceof Error && err.name === "AbortError") return;
      setModels([]);
      setError("Cannot reach the model service. Please confirm the backend is running.");
    } finally {
      setLoading(false);
    }
  }

  // When a provider is selected, only show categories that provider exposes
    ...
    <div className="models-page" style={{ padding: "32px 44px", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ marginBottom: 28, paddingBottom: 24, borderBottom: "1px solid var(--border)" }}>
        ...
        <p style={{ fontSize: 13.5, color: "var(--text-secondary)", maxWidth: 720, margin: 0 }}>
          Browse the full AI catalog — text, reasoning, vision, coding, image, video, embeddings and more.
        </p>
      </div>
              ...
              >
                ...
                <div className="model-card-stats" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, padding: "10px 0 0", borderTop: "1px solid var(--border)" }}>
                  {(() => {
                    ...
                    if (isMedia) {
                      const unit = model.pricingType === "per-second" ? "/s" : "/img";
                      return [
                    ...
                    if (model.tokenPricingTiers && model.tokenPricingTiers.length > 0) {
                      return [
                        { label: "Context", value: formatTokens(model.contextLength), color: "var(--text-primary)" },
                        { label: "Tier 1 input", value: `$${model.promptPrice}/M`, color: "var(--success)" },
                        { label: "Tier 1 output", value: `$${model.completionPrice}/M`, color: "var(--warning)" },
                      ];
                    }
                    return [
                      { label: "Context", value: formatTokens(model.contextLength), color: "var(--text-primary)" },
